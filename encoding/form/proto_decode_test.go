...
func TestDecodeValues(t *testing.T) {
	form, err := url.ParseQuery("a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration=" +
		...
		"simples=5566&string=go-cnf&timestamp=1970-01-01T00%3A00%3A20.000000002Z&uint32=32&uint64=64&very_simple.component=5566")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPopulateMapField(t *testing.T) {
	query, err := url.ParseQuery("map%5Bcnf%5D=https://go-cnf.dev/")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
	...
	err = populateMapField(field, comp.ProtoReflect().Mutable(field).Map(), []string{"map[cnf]"}, query["map[cnf]"])
	if err != nil {
		t.Fatal(err)
	}
	// Get the comp map field value
	if query["map[cnf]"][0] != comp.Map["cnf"] {
		t.Errorf("want: %s, got: %s", query["map[cnf]"], comp.Map["cnf"])
	}
}

func TestPopulateMapSepField(t *testing.T) {
	query, err := url.ParseQuery("map.name=cnf")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
...
func TestParseURLQueryMapKey(t *testing.T) {
	tests := []struct {
		...
		{
			fieldName: "map[cnf]", field: "map", fieldKey: "cnf", err: nil,
		},
		{
		...
		{
			fieldName: "map[cnf]=", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
			fieldName: "[cnf]", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
