...
import (
	...
	bdtest "github.com/chnxq/xkitmod/testdata/binding"
	"github.com/chnxq/xkitmod/testdata/complex"
	ectest "github.com/chnxq/xkitmod/testdata/encoding"
)

// This variable can be replaced with -ldflags like below:
// go test "-ldflags=-X github.com/chnxq/xkitmod/encoding/form.tagNameTest=form"
...
func TestFormCodecMarshal(t *testing.T) {
	...
	if !reflect.DeepEqual([]byte("password=cnf_pwd&username=cnf"), content) {
		t.Errorf("expect %s, got %s", "password=cnf_pwd&username=cnf", content)
	}
	...
	if !reflect.DeepEqual([]byte("username=cnf"), content) {
		t.Errorf("expect %s, got %s", "username=cnf", content)
	}
	...
	if !reflect.DeepEqual([]byte("id=1&name=cnf"), content) {
		t.Errorf("expect %s, got %s", "id=1&name=cnf", content)
	}
}
...
func TestFormCodecUnmarshal(t *testing.T) {
	...
	if !reflect.DeepEqual("cnf", bindReq.Username) {
		t.Errorf("expect %v, got %v", "cnf", bindReq.Username)
	}
	if !reflect.DeepEqual("cnf_pwd", bindReq.Password) {
		t.Errorf("expect %v, got %v", "cnf_pwd", bindReq.Password)
	}
}
...
func TestProtoEncodeDecode(t *testing.T) {
	in := &complex.Complex{
		...
		D:       22.22,
		Byte:    []byte("123"),
		Map:     map[string]string{"cnf": "https://go-cnf.dev/", "cnf_start": "https://go-cnf.dev/en/docs/getting-started/start/"},
		...
		String_:   &wrapperspb.StringValue{Value: "go-cnf"},
	...
	if "a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration="+
		...
		"map%5Bcnf%5D=https%3A%2F%2Fgo-cnf.dev%2F&map%5Bcnf_start%5D=https%3A%2F%2Fgo-cnf.dev%2Fen%2Fdocs%2Fgetting-started%2Fstart%2F&"+
		"numberOne=2233&price=11.23&sex=woman&simples=3344&simples=5566&string=go-cnf"+
