...
func TestEnvWithPrefix(t *testing.T) {
	...
	// set env
	prefix1, prefix2 := "CNF_", "FOO"
	envs := map[string]string{
		prefix1 + "SERVICE_NAME": "cnf_app",
...
func Test_env_load(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []*config.KeyValue{
				{Key: "SERVICE_NAME", Value: []byte("cnf_app"), Format: ""},
...

		{
			...
			args: args{
				envStrings: []string{
					"__SERVICE_NAME=cnf_app",
					"__ADDR=192.168.0.1",
					"__AGE=20",
				},
			},
			want: []*config.KeyValue{
				{Key: "_SERVICE_NAME", Value: []byte("cnf_app"), Format: ""},
...

		{
			name: "with prefixes",
			fields: fields{
				prefixes: []string{"CNF_", "FOO"},
			},
			args: args{
				envStrings: []string{
					"CNF_SERVICE_NAME=cnf_app",
					"CNF_ADDR=192.168.0.1",
					"FOO_AGE=20",
				},
			},
			want: []*config.KeyValue{
